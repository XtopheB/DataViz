...
The implicit null hypothesis $H_0$ tested can obviously vary and should be specific to  the type of graphic and the data set under scrutiny, but there is a clear parallelism between quantitative testing and visual testing, or first step discovery. When analysing and visualizing a data set, any data analysts or statistician implicitly perform this visual test. This is also what any viewer, not aware of the power of statistics, does when looking at any graphic. 
...
One of the most common and well-documented techniques for misleading with graphics is the truncation of the vertical axis, especially in bar charts. As highlighted in Figure \ref{Truncating1}  when the baseline does not start at zero, differences between values appear exaggerated, distorting the viewer’s perception of the magnitude of change \cite{Tufte2001,cairo2016truthful}. This issue is particularly critical for bar charts, where the length of the bar directly encodes the value. Truncating the axis violates the proportional relationship between data and its visual representation.

\begin{figure}[tbh]
\includegraphics[width = 0.5\linewidth]{Truncated_Bar_Graph.png} \includegraphics[width = 0.5\linewidth]{Bar_graph.png}
\label{Truncating1}
